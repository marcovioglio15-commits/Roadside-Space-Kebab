...
namespace CatOnASkateboard.PlayerStudio.Editor
{
    ...
    internal static class PlayerInputActionMenu
    {
        ...
        /// <param name="name">Serialized role whose compatible actions are listed.</param>
        internal static void Draw(SerializedObject serialized, string name)
        {
            ...
            if (!menus.TryGetValue(name, out Choices choices))
                menus.Add(name, choices = Build(name));
        ...
        /// <param name="role">Preset field selecting the required action shape.</param>
        /// <returns>Compatible persistent references and their unambiguous labels.</returns>
        private static Choices Build(string role)
        {
            ...
            foreach (string guid in AssetDatabase.FindAssets("t:InputActionAsset"))
            {
                ...
                foreach (UnityEngine.Object asset in AssetDatabase.LoadAllAssetsAtPath(path))
                    if (asset is InputActionReference reference && (reference.hideFlags & HideFlags.HideInHierarchy) == 0
                        && IsCompatible(reference.action, role))
        ...
        /// <summary>Filters buttons, continuous movement and the two distinct look units.</summary>
        /// <param name="action">Imported action configuration.</param>
        /// <param name="role">Requested preset field.</param>
        /// <returns>True when the action can supply that role without mixing displacement and rate bindings.</returns>
        private static bool IsCompatible(InputAction action, string role)
        {
            // Control layout inspection uses binding paths, so disconnected devices are still selectable.
            if (action == null)
                return false;
